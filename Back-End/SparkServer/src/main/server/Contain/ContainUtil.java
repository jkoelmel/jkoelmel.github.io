    ...
    public static Integer createWorkout(Request request) {
        try {
            ...
            for (int i = 0; i < (request.queryParamsValues("exercise_id").length); i++) {
                Exercise exercise = new Exercise(Integer.parseInt(request.queryParamsValues("exercise_id")[i]));
                exercise.getExercise();
                try {
                    Exercise newExercise = new Exercise(null);
                    newExercise.createExercise(exercise.getexercise_url(), exercise.getTitle(),
                            request.queryParamsValues("description")[i], exercise.getTags());
