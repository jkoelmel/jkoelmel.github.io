...
const CurrentWorkout = ({selectedPatient, setSelectedPatient}) => {
    ...
    const fetchWorkoutInfo = () => {
        axios.get('api/patient/workout/id', {
            params: {
                patient: selectedPatient
            }
        }).then((response) => {

            setWorkout(response.data.map((w) => {
    ...
    return (
        <div className={classes.root}>
            <List className={classes.paper} aria-label="workout-list"
                  style={{maxHeight: 300}}>
                <ListItem className={classes.modal}><u><b>Current Assignment</b></u></ListItem>
                {workout.map((w) => (
                    <div>
                        <ListItem
                            key={w.workout}
